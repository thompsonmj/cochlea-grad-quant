...
nEmbryos = size(psmadStruct.smooth,2);
nPts = size(psmadStruct.smooth{1,1},2);
                  
% Rescales data to [0,1] interval
%%%x_scaled = (0:0.02:1)'; % Replaced with below
x_scaled = (0:1/(nPts-1):1)';
%%%dpp_matrix = zeros(51,nEmbryos);
dpp_noise_matrix = zeros(nPts,nEmbryos);
dpp_extr_matrix = zeros(nPts,nEmbryos);
dpp_extr_matrix2 = zeros(nPts,nEmbryos);
dpp_int_matrix = zeros(nPts,nEmbryos);
...
% Specific call to normalization algorithms
    for i = 1:nEmbryos
        ...
        Data(:,i) = psmadStruct.smooth{1,i}(1,:)';
    end
