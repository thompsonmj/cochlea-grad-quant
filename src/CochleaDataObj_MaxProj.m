classdef CochleaDataObj_MaxProj
% Structure:
...
methods
%% Constructor
function C = CochleaDataObj_MaxProj(varargin)
...
%             end
    ...
    for iSec = 1:nSecs
        ...
        % Take the maximum intensity across optical sections at each
        % position.
        MaxProj = maxintproj(RawDat);
%         for iTgt = 1:nTgts
%             % Test functionality
%             figure
%             plot(MaxProj.(T{iTgt}),'Color','k','LineWidth',2)
%             hold on
...
%         end
        ...
        for iTgt = 1:nTgts
            if ~isequal(T{iTgt}, 'TOPRO3')
                ...
                minDat = min( SmMaxProj.(T{iTgt}) );
                SmMaxProj.(T{iTgt}) = SmMaxProj.(T{iTgt}) - minDat;
%                 RawDat.(T{iTgt}) = RawDat.(T{iTgt}) - minDat;
                C.Dat.(BAIdx{iSec}).OSR.(T{iTgt}).rawDat = ...
                    RawDat.(T{iTgt}) - minDat;
            else
        ...
        % ...
        algnMode = 'com';
        PERCENTILE = 1/4; % Use top PERCENTILE of data for alignment.
        
%         DatToAlgn.SmNorm = NormDatChiSq;
        DatToAlgn.MaxProj = SmMaxProj;
%         template = meanNormDat.pSmad;
        template = SmMaxProj.pSmad;
        AlDat = ...
...
%         end
        ...
        for iTgt = 1:nTgts
            ...
            C.Dat.(BAIdx{iSec}).USR.(T{iTgt}).algnDat = ...
                uniformsample( AlDat.MaxProj.(T{iTgt}), NSAMPLES );
        end
    ...
    %% Calculate mean profile for each organ, separated by apical/basal halves.
    % NOT WORKING 2019-04-03
%     S = fieldnames(C.Dat);
%     nSecs = numel(S);
%     basalProfs = struct;
%     apicalProfs = struct;
...
%                     basalProfs.(T{iTgt}) = ...
%                         [basalProfs.(T{iTgt}), ...
%                         C.Dat.(S{iSec}).USR.(T{iTgt}).algnDat];
%                 end
%             elseif S{iSec}(1) == 'A'
%                 % add to A list
%                 try
...
%     end
%     
%     T = fieldnames(basalProfs);
%     nTgts = numel(T);
%     for iTgt = 1:nTgts
%         C.Bmn.(T{iTgt}) = mean(basalProfs.(T{iTgt}),2);
%         C.Amn.(T{iTgt}) = mean(apicalProfs.(T{iTgt}),2);
%         C.Bstd.(T{iTgt}) = std(basalProfs.(T{iTgt}),2);
%         C.Astd.(T{iTgt}) = std(apicalProfs.(T{iTgt}),2);
%     end
%     
