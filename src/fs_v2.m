...
currentFolder = pwd;
[FileName,PathName] = uigetfile('*.xlsx;*.xls', ...                        % Only allow .xlsx and .xls files to be selected.
...
for i = 1:n                                                                % Iterate over all sheets (channels).
    ...
    %%%%%
    smoothsheet = strcat('Smooth','_',sheet);                              % Smoothened Data Sheet in Excel.
...
newfname = strcat(f(1),'_Smooth.',f(2));                                   % Add '_Smoothened' to the file name.
